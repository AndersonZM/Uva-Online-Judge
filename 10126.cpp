...
{    int d;
     if (p->nf == n){ cout << t << endl; no = false;}//Verifica se no node tem uma palavra, se sim imprime ela e o numero de ocorrencia dela
     for (d=0; d < 26; d++)
        if (p->pont[d] != NULL)
        {
            t += 'a' + d;
...
int main()
{
    ...
    while(cin >> n){
        ...
        while(true){
            ...
            istringstream ss;
            ss.str(linha);
            while(ss >> gS) { InsereTrie(); }
        }
        no = true;
        gS.clear();  ImprimePalavrasTrie(gT, gS);
        if(no) cout << "There is no such word." << endl;
    }
